...
github_proxy="https://proxy-github.higher-stark.workers.dev/"
...
Download_brook(){
	...
	cd ${file}
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -N "${github_proxy}https://github.com/txthinking/brook/releases/download/${brook_new_ver}/brook_linux_amd64"
		mv brook_linux_amd64 brook
	else
		wget --no-check-certificate -N "${github_proxy}https://github.com/txthinking/brook/releases/download/${brook_new_ver}/brook_linux_386"
...
Service_brook(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate ${github_proxy}https://raw.githubusercontent.com/Higher-Stark/brook-pf/main/brook-pf_centos -O /etc/init.d/brook-pf; then
		...
		if ! wget --no-check-certificate ${github_proxy}https://raw.githubusercontent.com/Higher-Stark/brook-pf/main/brook-pf_debian -O /etc/init.d/brook-pf; then
			echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/brook-pf
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "${github_proxy}https://raw.githubusercontent.com/Higher-Stark/brook-pf/main/brook-pf.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/brook-pf" ]]; then
		rm -rf /etc/init.d/brook-pf
		Service_brook
	fi
	wget -N --no-check-certificate "${github_proxy}https://raw.githubusercontent.com/Higher-Stark/brook-pf/main/brook-pf.sh" && chmod +x brook.sh
